...
class FlexSummaryAttention(nn.Module):
    ...
    def forward(
        ...
        # 统一 dtype (FlexAttention backward 对 dtype 敏感)
        # 强制使用 BF16 避免 autocast 后 dtype 不一致
        target_dtype = torch.bfloat16 if sum_q.is_cuda else sum_q.dtype
        sum_q = sum_q.to(target_dtype)
        ...
        # 统一 dtype (FlexAttention backward 对 dtype 敏感)
        # 强制使用 BF16 避免 autocast 后 dtype 不一致
        target_dtype = torch.bfloat16 if reg_q.is_cuda else reg_q.dtype
        reg_q = reg_q.to(target_dtype)
