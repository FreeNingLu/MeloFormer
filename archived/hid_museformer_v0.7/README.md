...
```bash
# 解压到服务器
cd ~/autodl-tmp && tar -xzvf hid_museformer_v0.7.tar.gz
```
...
```bash
cd ~/autodl-tmp/hid_museformer_v0.7 && python train.py \
...
```bash
cd ~/autodl-tmp/hid_museformer_v0.7 && torchrun --nproc_per_node=8 train.py \
    --data_dir ~/autodl-tmp/processed_data \
    --model_size large \
    --max_seq_len 24576 \
    --max_bars 2048 \
