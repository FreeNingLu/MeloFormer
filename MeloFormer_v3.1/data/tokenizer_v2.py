...
# -*- coding: utf-8 -*-
"""
HID Tokenizer V2 - 使用和弦 (Chord) 作为结构单元

每个和弦 token (Cmaj, Amin, G7 等) 同时表示结构边界和和声信息。
保留原有的位置、音高、乐器编码。
"""
...
class HIDTokenizerV2:
    ...
    总计: 6 + 217 + 16 + 128 + 129 + 32 + 19 + 64 + 32 = 643 tokens
    """

    POSITIONS_PER_CHORD = 16
    TEMPO_BINS = 32
    MIN_BPM = 40
    MAX_BPM = 250
    ...
    # FIM (Fill-In-the-Middle) 特殊 Token
    # 原始 vocab_size = 643 (ID 0-642)，FIM Token 使用 643-650
    FIM_TOKENS = {
        'TRK_PRE': 643,   # Track Mask: 目标轨道之前
        'TRK_SUF': 644,   # Track Mask: 目标轨道之后
        'TRK_MID': 645,   # Track Mask: 生成完整轨道
        'TIME_PRE': 646,  # Time FIM: 时间缺口之前
        'TIME_SUF': 647,  # Time FIM: 时间缺口之后
        'TIME_MID': 648,  # Time FIM: 生成目标时间段
        'SKE': 649,        # Skeleton FIM: 骨架条件
        'SKE_MID': 650,    # Skeleton FIM: 按骨架生成音符
    }
    FIM_VOCAB_SIZE = 651   # 643 + 8 FIM tokens
    ...
    @property
    def fim_vocab_size(self) -> int:
        """FIM 模式下的词表大小 (含 8 个 FIM 特殊 token)"""
        return self.FIM_VOCAB_SIZE
    ...
    def encode_with_info(
        ...
        # 解析和弦映射
        chord_map = {}  # {(chord_idx, pos): chord_name}
        ...
        for line in lines:
            ...
            if line_strip.startswith('CHORDS '):
                ...
                for part in parts:
                    if ':' in part:
                        chord_pos, chord = part.split(':', 1)
                        if '.' in chord_pos:
                            chord_str, pos_str = chord_pos.split('.', 1)
                            chord_idx = int(chord_str)
                            pos = int(pos_str)
                        else:
                            chord_idx = int(chord_pos)
                            pos = 0
                        chord_map[(chord_idx, pos)] = chord
                break
        ...
        for line in lines:
            ...
            for token in tokens_in_line:
                # Chord marker: B0, B1, ... (TXT 格式中的 chord 编号)
                if token.startswith('B') and token[1:].isdigit():
                    current_chord = int(token[1:])
                    # 查找和弦映射，优先用 pos=0 的和弦
                    chord_token = chord_map.get((current_chord, 0), 'N')
                    # 确保和弦 token 在词表中
                ...
                if token.startswith('T') and token[1:].isdigit():
                    pos = int(token[1:])
                    current_position = min(pos, self.POSITIONS_PER_CHORD - 1)
    ...
    def encode_midi_with_chords(
        ...
        for inst_id, notes in sorted(notes_by_instrument.items()):
            ...
            for note in notes_sorted:
                ...
                pos_16th = int(pos_in_window * self.POSITIONS_PER_CHORD / window_ticks)
                pos_16th = max(0, min(self.POSITIONS_PER_CHORD - 1, pos_16th))
    ...
    def decode(self, token_ids: List[int], skip_special: bool = True) -> str:
        ...
        lines = []
        current_track = None
        current_chord = -1  # chord 计数器（每个乐器独立）
        note_line = []
        ...
        for token_id in token_ids:
            ...
            # 和弦 token → B{chord} 格式输出
            if self.is_chord_token(token_id):
                if note_line:
                    lines.append(' '.join(note_line))
                current_chord += 1
                note_line = [f'B{current_chord}']  # 输出 B{chord} 格式
                continue
...
if __name__ == '__main__':
    ...
    print(f"  和弦 tokens: 217 (N + 12根音 × 18类型)")
