...
class MusicGenerator:
    ...
    def generate(
        ...
        for step in range(max_length - len(prompt_ids)):
            ...
            if self.tokenizer.is_chord_token(next_token.item()):
                new_chord_id = min(new_chord_id + 1, self.model.max_bars - 1)
    ...
    def tokens_to_midi(
        ...
        # 当前音符的 duration 和 velocity（用于后续 P token）
        current_duration = ticks_per_beat // 2  # 默认八分音符
        current_velocity = 80  # 默认 velocity
        ...
        for tid in token_ids:
            ...
            if token.startswith('P') and token[1:].isdigit():
                ...
                # 创建待处理音符（等待 L/V token 更新）
                pending_note = {
                    'pitch': pitch,
                    'start': pos_tick,
                    'duration': current_duration,  # 使用上一个音符的 duration
                    'velocity': current_velocity,  # 使用上一个音符的 velocity
                    'instrument': current_instrument,
                }
                continue

            # Duration token: L1-L64
...
def load_model(checkpoint_path: str, device: torch.device) -> tuple:
    ...
    model = create_model(
        ...
        max_bars=args.get('max_bars', 2048),
...
def parse_args():
    parser = argparse.ArgumentParser(description='MeloFormer 音乐生成')
